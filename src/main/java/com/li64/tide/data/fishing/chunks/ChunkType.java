...
public interface ChunkType {
    ...
    DevilsHoleChunk DEVILS_HOLE_CHUNK = CompatHelper.isAlexsMobsLoaded() ? register(new DevilsHoleChunk()) : null;
    ...
    static ChunkType get(ResourceLocation id) {
        return TYPES.stream()
            .filter(chunkType -> chunkType.id().equals(id))
